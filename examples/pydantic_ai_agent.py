...
Needs the `examples` dependency group: `uv sync --group examples`, or
`uv add "pydantic-ai-slim[mcp]"` directly. Also needs an API key for
whichever model you point `Agent(...)` at - Anthropic's, by default here
...
Run either with `python -m examples.pydantic_ai_agent`.
"""
