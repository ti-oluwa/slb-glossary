...
def _rank_of_first_hit(ranked_terms: typing.Sequence[str], expected: frozenset[str]) -> int | None:
    """
    1-based rank of the first term in `ranked_terms` that's in `expected`, or `None`.
...
def recall_at_k(
    ...
    """
    Fraction of queries with at least one relevant term in their top `k` results.
    ...
    hits = sum(
        ...
        if _rank_of_first_hit(ranked_terms[:k], expected) is not None
    )
...
def mean_reciprocal_rank(
    ...
    """
    Mean, across queries, of `1 / rank` of the first relevant result (`0` if none found).
    ...
    reciprocal_ranks = []
    for ranked_terms, expected in zip(results, expecteds, strict=True):
        rank = _rank_of_first_hit(ranked_terms, expected)
        reciprocal_ranks.append(1.0 / rank if rank else 0.0)
    return sum(reciprocal_ranks) / len(reciprocal_ranks)


def _dcg_at_k(ranked_terms: typing.Sequence[str], expected: frozenset[str], k: int) -> float:
    """
...
def ndcg_at_k(
    ...
    """
    Mean, across queries, of normalized DCG@k with binary relevance
    (every expected term worth the same).

    The ideal DCG for a query with any expected terms at all is
    `1 / log2(2) == 1.0` (the best possible ranking puts a relevant
    ...
    for ranked_terms, expected in zip(results, expecteds, strict=True):
        ...
        dcg = _dcg_at_k(ranked_terms, expected, k)
