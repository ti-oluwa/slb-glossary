...
class TestEnv:
    def test_returns_default_when_var_unset(self, monkeypatch: pytest.MonkeyPatch):
    ...
    def test_casts_falsy_strings_to_bool_false(self, monkeypatch: pytest.MonkeyPatch, raw: str):
        """Unrecognized/falsy string forms cast to `False` for a `bool` default."""
        monkeypatch.setenv(ENV_VAR, raw)
        assert env(ENV_VAR, True) is False

    def test_casts_to_int(self, monkeypatch: pytest.MonkeyPatch):
        """A numeric string casts to `int` when `default` is an `int`."""
        monkeypatch.setenv(ENV_VAR, "42")
        assert env(ENV_VAR, 0) == 42

    def test_casts_to_float(self, monkeypatch: pytest.MonkeyPatch):
    ...
    def test_casts_to_str(self, monkeypatch: pytest.MonkeyPatch):
...
class TestParseInt:
    ...
    def test_parses_valid_int_string(self, text: str, expected: int):
...
class TestSplitExclude:
    ...
    def test_term_names_are_normalized_case_and_whitespace_insensitively(self):
    ...
    def test_empty_or_none_input_returns_empty_collections(self):
    ...
    def test_falsy_entries_within_the_collection_are_skipped(self):
