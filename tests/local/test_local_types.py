...
class TestMetadata:
    def test_load_returns_defaults_when_file_missing(self, tmp_path: pathlib.Path):
        """`Metadata.load()` on a nonexistent path returns fresh defaults, no error."""
        metadata = Metadata.load(tmp_path / "missing.json")
        assert metadata == Metadata()

    def test_save_then_load_round_trips(self, tmp_path: pathlib.Path):
    ...
    def test_save_creates_missing_parent_directories(self, tmp_path: pathlib.Path):
        """`save` creates missing parent directories before writing."""
        path = tmp_path / "nested" / "dir" / "metadata.json"
        Metadata().save(path)
        assert path.exists()

    def test_load_ignores_unknown_keys(self, tmp_path: pathlib.Path):
    ...
    def test_load_fills_missing_fields_with_defaults(self, tmp_path: pathlib.Path):
    ...
    def test_is_a_plain_dataclass_with_expected_fields(self):
