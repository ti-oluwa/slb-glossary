...
class TestGetField:
    ...
    def test_returns_none_for_none_or_empty_value(self, value):
        """A matched key whose value is `None`/`""` counts as absent, returns `None`."""
        assert get_field({"term": value}, "term") is None

    def test_falsy_but_not_empty_values_are_returned(self):
...
class TestParseRelated:
    ...
    def test_skips_malformed_items_but_keeps_valid_ones(self):
...
class TestRecordToResult:
    ...
    def test_synthesizes_a_local_url_when_url_field_missing_value(self):
    ...
    def test_synthesizes_a_local_url_when_url_field_is_none(self):
    ...
    def test_language_field_none_always_uses_default_language(self):
    ...
    def test_empty_language_value_falls_back_to_default_language(self):
    ...
    def test_field_set_to_none_leaves_that_slot_unset(self):
    ...
    def test_unparsable_related_field_leaves_related_none(self):
...
@pytest.mark.anyio
class TestLoadFile:
    async def test_imports_csv_rows(self, db, tmp_path: pathlib.Path):
    ...
    async def test_imports_json_rows(self, db, tmp_path: pathlib.Path):
    ...
    async def test_format_argument_overrides_path_extension(self, db, tmp_path: pathlib.Path):
    ...
    async def test_raises_database_error_for_unsupported_format(self, db, tmp_path: pathlib.Path):
    ...
    async def test_raises_value_error_for_batch_size_below_one(self, db, tmp_path: pathlib.Path):
    ...
    async def test_rows_with_no_term_are_skipped(self, db, tmp_path: pathlib.Path):
    ...
    async def test_flushes_remaining_partial_batch_at_the_end(self, db, tmp_path: pathlib.Path):
    ...
    async def test_source_tag_defaults_to_user(self, db, tmp_path: pathlib.Path):
        """
    ...
    async def test_reader_error_is_wrapped_in_database_error(
        self, db, tmp_path: pathlib.Path, monkeypatch: pytest.MonkeyPatch
    ):
        ...
        def broken_read_rows(path, format=None):
            yield {"term": "Porosity"}
    ...
    async def test_processing_error_propagates_unwrapped(
        ...
        def broken_record_to_result(row, **kwargs):
    ...
    async def test_flushes_buffer_on_processing_error_via_finally(
        ...
        def flaky_record_to_result(row, **kwargs):
            nonlocal call_count
            call_count += 1
            if call_count == 2:
                raise ValueError("boom")
