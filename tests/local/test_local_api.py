...
class TestDumpLoadRelated:
    def test_round_trips_related_terms(self):
    ...
    def test_dump_none_or_empty_returns_none(self):
    ...
    def test_load_none_or_empty_returns_none(self):
...
class TestApplyExclude:
    def test_no_clause_added_when_exclude_is_none(self):
    ...
    def test_adds_url_not_in_clause(self):
    ...
    def test_adds_term_name_not_in_clause_normalized(self):
...
class TestFuzzyMatchTopics:
    def test_empty_topic_returns_empty_string(self):
    ...
    def test_exact_case_insensitive_match_returns_original_casing(self):
    ...
    def test_several_comma_separated_parts_resolve_independently(self):
    ...
    def test_duplicate_resolved_topics_are_deduplicated(self):
...
class TestUpsertResults:
    async def test_writes_results_and_returns_count(self, db):
    ...
    async def test_skips_results_with_no_url(self, db):
    ...
    async def test_empty_input_writes_nothing(self, db):
        """An empty `results` writes nothing and returns `0`."""
        assert await upsert_results(db, []) == 0

    async def test_conflicting_url_and_topic_upserts_in_place(self, db):
    ...
    async def test_language_override_forces_every_result_to_that_language(self, db):
...
class TestUpsertResultsIncrementally:
    async def test_yields_every_result_unchanged(self, db):
    ...
    async def test_flushes_in_batches_of_batch_size(self, db):
    ...
    async def test_flushes_remainder_at_the_end(self, db):
    ...
    async def test_stats_dict_populated_with_written_and_batches(self, db):
    ...
    async def test_persist_on_error_true_flushes_buffered_results_before_raising(self, db):
    ...
    async def test_raises_value_error_for_batch_size_below_one(self, db):
...
class TestSearchDispatch:
    async def test_lexical_mode_calls_lexical_search(self, db, monkeypatch: pytest.MonkeyPatch):
    ...
    async def test_semantic_mode_calls_vector_search(self, db, monkeypatch: pytest.MonkeyPatch):
    ...
    async def test_hybrid_mode_calls_hybrid_search(self, db, monkeypatch: pytest.MonkeyPatch):
    ...
    async def test_mode_accepts_searchmode_enum_member(self, db, monkeypatch: pytest.MonkeyPatch):
    ...
    async def test_scored_false_strips_scores(self, db, monkeypatch: pytest.MonkeyPatch):
    ...
    async def test_scored_true_keeps_scores(self, db, monkeypatch: pytest.MonkeyPatch):
...
class TestResolveTopic:
    async def test_none_or_empty_topic_returns_none(self, db):
    ...
    async def test_non_fuzzy_returns_topic_unchanged(self, db):
    ...
    async def test_fuzzy_resolves_against_stored_topics(self, db):
    ...
    async def test_fuzzy_with_no_match_returns_none(self, db):
...
class TestGetTermsOn:
    async def test_returns_terms_filed_under_topic(self, db):
    ...
    async def test_unresolved_topic_returns_empty_list(self, db):
    ...
    async def test_excludes_given_urls_and_names(self, db):
...
@pytest.mark.anyio
class TestIterTerms:
    async def test_streams_every_term_ordered_by_name(self, db):
    ...
    async def test_respects_batch_size_across_multiple_fetches(self, db):
    ...
    async def test_filters_by_topic_start_letter_and_language(self, db):
...
@pytest.mark.anyio
class TestGetTerm:
    async def test_finds_by_exact_url(self, db):
    ...
    async def test_topic_disambiguates_multiple_definitions(self, db):
    ...
    async def test_with_similar_and_no_exact_match_still_returns_alternatives(self, db):
...
class TestGetTermDefinitions:
    async def test_returns_every_definition_ordered_by_topic(self, db):
    ...
    async def test_returns_empty_list_when_not_found(self, db):
...
class TestGetRandomTerm:
    async def test_returns_none_for_empty_database(self, db):
        """Returns `None` when the local database has no terms at all."""
        assert await get_random_term(db) is None

    async def test_returns_a_stored_term(self, db):
...
class TestGetTermsUrls:
    async def test_returns_matching_urls_without_query(self, db):
    ...
    async def test_with_query_delegates_to_search(self, db):
    ...
    async def test_excludes_results_with_no_url(self, db):
...
@pytest.mark.anyio
class TestGetTopics:
    async def test_returns_topic_to_term_count(self, db):
    ...
    async def test_excludes_empty_topic(self, db):
...
@pytest.mark.anyio
class TestCount:
    async def test_returns_zero_for_empty_database(self, db):
        """Returns `0` for a freshly created database."""
        assert await count(db) == 0

    async def test_returns_total_row_count(self, db):
