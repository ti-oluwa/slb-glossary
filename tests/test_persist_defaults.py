...
class TestPersistKwargs:
    ...
    def test_explicit_cache_results_true_is_used_as_is(self) -> None:
        """An explicit `--cache` (`True`) always wins, regardless of the constant."""
        constants.cli_cache_by_default = False
        kwargs = persist_kwargs({"cache_results": True})
        assert kwargs["persist"] is True

    def test_explicit_cache_results_false_is_used_as_is(self) -> None:
    ...
    def test_missing_cache_results_key_also_resolves_via_constant(self) -> None:
