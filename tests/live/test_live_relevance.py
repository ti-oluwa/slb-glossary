...
class TestScoreNameMatch:
    def test_exact_match_scores_exact_match_score(self):
    ...
    def test_prefix_match_scores_prefix_match_score(self):
    ...
    def test_empty_query_or_term_returns_none(self):
...
class TestScoreContentOverlap:
    def test_full_coverage_scores_content_match_score_cap(self):
    ...
    def test_no_coverage_scores_zero(self):
        ...
        assert score_content_overlap("xyz", "A rock property") == 0.0

    def test_empty_query_scores_zero(self):
        """An empty (post-normalization) query scores `0.0`."""
        assert score_content_overlap("", "A rock property") == 0.0

    def test_no_texts_scores_zero(self):
        """No texts (or all-empty texts) scores `0.0`."""
        assert score_content_overlap("porosity") == 0.0
        assert score_content_overlap("porosity", "", None or "") == 0.0

    def test_falsy_texts_are_skipped_not_erroring(self):
        """A falsy text among several (e.g. an empty string) is skipped, not an error."""
        score = score_content_overlap("rock", "", "A rock property")
        assert score == constants.content_match_score_cap

    def test_longer_text_does_not_score_higher_for_repeating_the_token(self):
...
class TestScoreResult:
    def test_lexical_mode_uses_name_match_when_available(self):
    ...
    def test_lexical_mode_falls_back_to_content_overlap(self):
    ...
    def test_lexical_is_the_default_mode(self):
    ...
    def test_hybrid_mode_raises_value_error(self):
        ...
        with pytest.raises(ValueError, match="HYBRID"):
            score_result("porosity", result, mode=SearchMode.HYBRID)

    def test_semantic_mode_returns_cosine_similarity(self, monkeypatch: pytest.MonkeyPatch):
